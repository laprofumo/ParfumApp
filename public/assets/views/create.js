...
export async function renderCreate(root) {
  root.innerHTML = `
    ...
    <div class="card">
      <div class="row">
        <div>
          <label>Kundeninfo</label>
          <input id="kundeninfo" placeholder="Vorname Nachname" />
        </div>
        <div>
  ...
  async function doSave(askPrint=true) {
    ...
    const payload = {
      "Kundeninfo": getVal("kundeninfo").trim(),
    ...
    try {
      if (!lastSaved) {
        showModal({
          ...
          bodyHtml: "Vor dem Drucken muss gespeichert werden. Jetzt speichern?",
          buttons: [
            { text:"Ja", className:"primary", onClick: async () => { await doSave(false); await doPrintFirstCreation(); } },
            { text:"Nein" }
          ]
        });
        return;
  ...
  clearBtn.onclick = () => {
    ...
    try {
      ["kundeninfo","namefragrance","p1","d1","p2","d2","p3","d3","bem","g1","g2","g3","tot"].forEach(id => setVal(id,""));
      setVal("konz","EDP");
      setVal("format","15");
      lastSaved = null;
